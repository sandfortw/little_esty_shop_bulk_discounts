...
RSpec.describe Invoice, type: :model do
  ...
  describe 'instance methods' do
    before do
      ...
      @item_2 = Item.create!(name: 'Butterfly Clip', description: 'This holds up your hair but in a clip',
      ...
      @invoice_item1 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_1.id, quantity: 9, unit_price: 10,
                                           status: 2)
      @invoice_item2 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2.id, quantity: 1, unit_price: 10,
                                           status: 1)
    end
    it 'total_revenue' do
    ...
    describe '#merchant_total_revenue(merchant)' do
      it 'returns the total revenue of the merchant (without discounts)' do
        @invoice_item3 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2_1.id, quantity: 9,
                                             unit_price: 1, status: 2)
        @invoice_item4 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2_2.id, quantity: 1,
    ...
    describe '#merchant_total_revenue_discounted(merchant)' do
      ...
      it 'excludes items that are below the quantity threshold' do
        ...
        invoice_item4 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2_2.id, quantity: 1,
                                            unit_price: 1, status: 1)
        invoice_item5 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2.id, quantity: 10,
                                            unit_price: 10, status: 1)
    ...
    describe '#discount_total_revenue' do
      describe 'gives a discounted total for a merchant, based on quantity of invoice items, and percent off' do
        ...
        it 'another merchants with no discounts items are not being affected by discounts' do
          ...
          invoice_item3 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: item_3.id, quantity: 1,
                                              unit_price: 10, status: 1)
          BulkDiscount.create!(percent_discounted: 50, quantity_threshold: 5, merchant_id: @merchant1.id)
